...
void ldnrevcheck()
{
  ...
  if (a == 1)
  {
    digitalWrite(6, LOW);   //turn off my motor
    digitalWrite(7, LOW);
    bitWrite(status, 0, 0);
    bitWrite(status, 1, 0);
    bitWrite(status, 4, 0);
...
void rdnrevcheck()
{
  ...
  if (a == 1)
  {
    digitalWrite(8, LOW);   //turn off my motor
    digitalWrite(9, LOW);
    bitWrite(status, 2, 0);
    bitWrite(status, 3, 0);
    bitWrite(status, 6, 0);
...
void lup()
{
  ...
  { if (digitalRead(2) == HIGH)
    ...
    { a = (bitRead(status, 0));
      if (a == 0)
      { digitalWrite((6), HIGH); //all happy turn the motor on
        luprevcheck();
...
void rup()
{
  ...
  { if (digitalRead(4) == HIGH)
    ...
    { a = (bitRead(status, 2));
      if (a == 0)
      { digitalWrite((8), HIGH); //all happy turn the motor on
        ruprevcheck();
      }          //unless we are already running in the other direction -stop the motor and start again.
    }
  }
...
void ldn()
{
  ...
  { if (digitalRead(3) == HIGH)
    ...
    { a = (bitRead(status, 4));
      if (a == 0)
      { digitalWrite((7), HIGH); //all happy turn the motor on
        ldnrevcheck();
      }          //unless we are already running in the other direction -stop the motor and start again.
    }
  }
...
void rdn()
{
  ...
  { if (digitalRead(5) == HIGH)
    ...
    { a = (bitRead(status, 6));
      if (a == 0)
      { digitalWrite((9), HIGH); //all happy turn the motor on
        rdnrevcheck();
      }          //unless we are already running in the other direction -stop the motor and start again.
    }
  }
...
void offtest()
{
  ...
  if (digitalRead(11) == 0) // the ignition is on again
  {
    timeoutflag = 0;
    runoutflag = 0;
  }
